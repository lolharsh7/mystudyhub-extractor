...
def get_subjects(batch_id):
    ...
    subjects = []
    for div in soup.select("div.card-body a[href*='topics.php?courseid=']"):
        parts = urllib.parse.parse_qs(urllib.parse.urlparse(div['href']).query)
        subjects.append({
            "subject_name": div.text.strip(),
...
def get_topics(batch_id, subject_id):
    ...
    soup = BeautifulSoup(resp.text, 'html.parser')
    topics = []
    for div in soup.select("div.card-body a[href*='concepts.php?courseid=']"):
        parts = urllib.parse.parse_qs(urllib.parse.urlparse(div['href']).query)
        topics.append({
            "topic_name": div.text.strip(),
...
def get_classes(batch_id, subject_id, topic_id):
    ...
    soup = BeautifulSoup(resp.text, 'html.parser')
    classes = []
    for div in soup.select("div.card-body a[href*='watch.php?courseid=']"):
        parts = urllib.parse.parse_qs(urllib.parse.urlparse(div['href']).query)
        classes.append({
            "class_name": div.text.strip(),
            "class_id": parts['id'][0]
        })
    return classes
